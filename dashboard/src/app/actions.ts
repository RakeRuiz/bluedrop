...
export async function getFrancoGlobalEnabled(): Promise<boolean> {
  try {
    ...
    // Supabase en el paso de build de Docker) — se asume Franco activo por defecto.
    console.error('[actions] No se pudo leer el estado de Franco', error);
    return true;
  }
}

export async function setFrancoGlobalEnabled(formData: FormData) {
  const enabled = formData.get('enabled') === 'true';
  const supabase = getSupabaseServerClient();

  await supabase.from('bluedrop_app_settings').upsert({ id: true, franco_enabled: enabled });
